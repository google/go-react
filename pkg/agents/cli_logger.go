...
func (l cliLogger[TOut]) Predict(
	...
	// Print out in green.
	if resp.Action != "" {
		fmt.Fprintf(l.out, "%sThought: %q Action: %q Input: %q%s\n",
			green, resp.Thought, resp.Action, resp.Input, reset)
	} else {
		fmt.Fprintf(l.out, "%sThought: %q FinalAnswer: %v%s\n",
			green, resp.Thought, resp.FinalAnswer, reset)
	}
	return resp, nil
}
